function predictedLabels = predictLabelsFromTestSamples(testingSamples, lookUpTable, testLabelOccurrences)

    predictedLabels = zeros(2300,1);
    predictedLabels(:,1) = 8; % initialise labels to garbage value, USED FOR DEBUGGING
    
    numberOfClass0Labels = testLabelOccurrences(1);
    numberOfClass1Labels = testLabelOccurrences(2);
    numberOfClass2Labels = testLabelOccurrences(3);
    ...
    fprintf('\nClassifying given testing samples ...\n\n');
    
    
    for sampleNumber = 1:2300 % for all testing samples, could have called this 'current row'
	    
        % fprintf('Classifying testing sample %d ...\n', sampleNumber);
        ...
        for featureNumber = 1:57 
            ...
            discreteValueFromTestSample = testingSamples(sampleNumber,featureNumber);
        ...
        % assign class-label to the given testing-sample (i.e. the given feature vector)
